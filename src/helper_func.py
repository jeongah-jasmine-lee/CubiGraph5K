"""
📝 Helper functions from CubiGraph5k
"""
...
def get_room_name(svg_group: bs4Tag) -> str:
    # ⭐Finds the text element with the class 'NameLabel' to get the room name
    text_tag = svg_group.find('g', attrs={'class': 'TextLabel NameLabel'})
...
class Room:
    ...
    def get_adjacent_doors(self, doors: List['Door'], file_path: str) -> None:
        room = self.to_shapely_polygon()

        error_log_path="error-case.txt"

        # 🔥 Logic made by Jasmine 🔥 Check if the room geometry is valid
        # Open the log file for writing error cases
        with open(error_log_path, "a") as error_file:
            if not room.is_valid:
                error_msg = f"{file_path}\t{self.name}\n"
                print("🔥" + error_msg)
                error_file.write(error_msg)
                room = room.buffer(0)  # Try to fix geometry
                if not room.is_valid:
                    error_msg = f"{self.name} Failed to Fix. Skipping.\n"
                    print(error_msg)
                    error_file.write(error_msg)
                    return  
...
class Plan:
    ...
    def generate_room_relation(self, file_path:str) -> None:
        for room in self.rooms:
            room.get_adjacent_doors(self.doors, file_path)
...
class ImagePlan:
    ...
    def __init__(self, svg: bs4Tag):
        self.rooms = [] # parse from svg
        self.name2room = defaultdict(list)  # ⭐
        self.name2index = {}
        self.doors = [] # parse from svg
        self.name2door = {}
        self.relation = [] # generate by calling self.generate_room_relation()
        ...
        for r in svg.find_all('g', attrs={'class': 'Space'}):
            ...
            room.name = room_name  # ⭐ Assign the room name from the SVG
            self.rooms.append(room)
            
            self.name2room[room_name].append(room) #self.name2room[room.name] = room
            #print("self.rooms: ", self.rooms)
            #print("self.name2room: ", self.name2room)
        
        #self.room_names = self.name2room.keys()
        self.room_names = [name for name, room_list in self.name2room.items() for _ in room_list]
...
def extract_connected_pairs(adjacency_list):
    door_connected_pairs = []

    # Iterate over each room in the adjacency list
    for room, connections in adjacency_list.items():
        # For each connection, check if it's door-connected (label 2)
        for connected_room, label in connections.items():
            #if label == 2:
            # To avoid duplicates, ensure we only add (room1, room2) once
            if (connected_room, room) not in door_connected_pairs:
                door_connected_pairs.append((room, connected_room))

    return door_connected_pairs

def find_all_reachable_pairs(adjacency_list):
    reachable_pairs = []
    
    # BFS helper function to find all reachable rooms from a given start room
    def bfs(start):
        visited = set()
        queue = deque([start])
        visited.add(start)
        ...
        while queue:
            ...
            for neighbor, _ in adjacency_list[current_room].items():
                if neighbor not in visited:
                    visited.add(neighbor)
                    queue.append(neighbor)
                    reachable_pairs.append((start, neighbor))  # Add pair directly to the flat list
    
    # Perform BFS from each room in the adjacency list
